...
import '../services/baylan_card_service.dart';
...
class _BaylanCardScreenState extends State<BaylanCardScreen> {
    ...
    BaylanCardService.onReadCard = (cardData, resultCode) {
      setState(() {
        ...
        if (cardData != null && resultCode == 'Success') {
          ...
          _reserveCreditController.text = _currentCard!.reserveCredit?.toString() ?? '';
          _criticalCreditController.text = _currentCard!.criticalCreditLimit?.toString() ?? '';
  ...
  Future<void> _requestPermissions() async {
    await [
      Permission.nearFieldCommunication,
  ...
  Future<void> _writeCard() async {
    ...
    try {
      await _baylanService.writeCard(
        ...
        reserveCreditLimit: double.tryParse(_reserveCreditController.text) ?? 0.0,
        criticalCreditLimit: double.tryParse(_criticalCreditController.text) ?? 0.0,
      );
    } catch (e) {
      setState(() {
        _isLoading = false;
                ...
                child: Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      const SizedBox(height: 16),
                      _buildCardInfoRow('Kart Seri No', _currentCard!.cardSeriNo),
                      _buildCardInfoRow('Müşteri No', _currentCard!.customerNo),
                      _buildCardInfoRow('Sayaç No', _currentCard!.meterNo),
                      _buildCardInfoRow('Ana Kredi', _currentCard!.mainCredit?.toStringAsFixed(2)),
                      _buildCardInfoRow('Rezerv Kredi', _currentCard!.reserveCredit?.toStringAsFixed(2)),
                      _buildCardInfoRow('Kritik Kredi Limiti', _currentCard!.criticalCreditLimit?.toStringAsFixed(2)),
                      _buildCardInfoRow('Batarya', _currentCard!.battery?.toStringAsFixed(1)),
                      _buildCardInfoRow('Toplam Tüketim', _currentCard!.totalConsumption?.toStringAsFixed(2)),
                      _buildCardInfoRow('Sayaçtaki Kalan Kredi', _currentCard!.remainingCreditOnMeter?.toStringAsFixed(2)),
                      if (_currentCard!.meterDate != null)
                        _buildCardInfoRow('Sayaç Tarihi', _currentCard!.meterDate!.toString().split(' ')[0]),
                      if (_currentCard!.lastCreditChargeDate != null)
                        _buildCardInfoRow('Son Kredi Yükleme', _currentCard!.lastCreditChargeDate!.toString().split(' ')[0]),
                    ],
                  ),
                ),
              ),
